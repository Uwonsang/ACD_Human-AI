...
class OvercookedState(object):
    ...
    def deepcopy(self):
        return OvercookedState(
            ...
            None if self.order_list is None else list(self.order_list), timestep=self.timestep)
...
class OvercookedGridworld(object):
    ...
    def from_layout_name(layout_name, **params_to_overwrite):
        ...
        base_layout_params = read_layout_dict(layout_name, params_to_overwrite['layouts_dir'])

        grid = base_layout_params['grid']
        del base_layout_params['grid']
        base_layout_params['layout_name'] = layout_name
        del params_to_overwrite['layouts_dir']
    ...
    def get_state_transition(self, state, joint_action):
        ...
        sparse_reward_by_agent, shaped_reward_by_agent, shaped_info_by_agent = self.resolve_interacts(new_state,
                                                                                                      joint_action)
    ...
    def lossless_state_encoding(self, overcooked_state, horizon=400, debug=False):
        ...
        variable_map_features = ["onions_in_pot", "onions_cook_time_remaining", "onion_soup_loc", "soup_done", "dishes", "onions"]
                ...
                if obj.name == "soup":
                    ...
                    if soup_type == "onion":
                        if obj.position in self.get_pot_locations():
                            soup_type, num_onions, cook_time = obj.state
                            if num_onions < self.num_items_for_soup: 
                                # onions_in_pot is used when the soup is idling, and ingredients could still be added
                                state_mask_dict["onions_in_pot"] += make_layer(obj.position, num_onions)
                            else:
                                state_mask_dict["onion_soup_loc"] += make_layer(obj.position, 1)
                                state_mask_dict["onions_cook_time_remaining"] += make_layer(obj.position, self.soup_cooking_time - cook_time)
    ...
    def featurize_state(self, overcooked_state: object, mlp: object) -> object:
