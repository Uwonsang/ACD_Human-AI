...
def parse_args():
    parser = argparse.ArgumentParser('For sim graph')
...
def overall_figure(result_dir, save_dir, args):
    ...
    if args.is_sparse:
        file_names = {
            '$Our^{s}$': 'scientific_reports_3.pbt_return_usesim vs Proxy_sparse.csv',
            '$Our^{-s}$': 'scientific_reports_3.pbt_return vs Proxy_sparse.csv',
            '$MAESTRO^{s}$': 'scientific_reports_3.pbt_td_usesim vs Proxy_sparse.csv',
            '$MAESTRO^{-s}$': 'scientific_reports_3.pbt_td vs Proxy_sparse.csv',
            '$PLR^{s}$': 'scientific_reports_2.plr_td_usesim vs Proxy_sparse.csv',
            '$PLR^{-s}$': 'scientific_reports_2.plr_td_usesim vs Proxy_sparse.csv'}
    else:
        file_names = {
            '$Our^{s}$': 'scientific_reports_3.pbt_return_usesim vs Proxy.csv',
            '$Our^{-s}$': 'scientific_reports_3.pbt_return vs Proxy.csv',
            '$MAESTRO^{s}$': 'scientific_reports_3.pbt_td_usesim vs Proxy.csv',
            '$MAESTRO^{-s}$': 'scientific_reports_3.pbt_td vs Proxy.csv',
            '$PLR^{s}$': 'scientific_reports_2.plr_td_usesim vs Proxy.csv',
            '$PLR^{-s}$': 'scientific_reports_2.plr_td_usesim vs Proxy.csv'}
    ...
    if args.figure_type == "split":
        ...
        colors = ['skyblue', 'lightcoral', 'gold', 'lightgreen', 'tomato', 'salmon']
        for i, (name, df) in enumerate(data.items()):
            plt.bar(index + i * bar_width, df['mean'], bar_width, label=name, color=colors[i], yerr=df['std'],
                    capsize=10)
        ...
        plt.legend()
        if args.is_sparse:
            plt.savefig(os.path.join(save_dir, 'compare_sim_split_sparse.pdf'), dpi=300)
        else:
            plt.savefig(os.path.join(save_dir, 'compare_sim_split.pdf'), dpi=300)
        ...
        plt.tight_layout()
        if args.is_sparse:
            plt.savefig(os.path.join(save_dir, 'compare_sim_full_sparse.pdf'), dpi=300)
        else:
            plt.savefig(os.path.join(save_dir, 'compare_sim_full.pdf'), dpi=300)
        # plt.show()
...
if __name__ == "__main__":
    ...
    save_dir = '/app/analysis/figure/similarity/'
