...
def parse_args():
    parser = argparse.ArgumentParser('For td_return graph')
...
def overall_figure(result_dir, save_dir, args):
    ...
    if args.figure_type == "split":
        ...
        colors = ['skyblue', 'lightcoral', 'darkorange', 'lightgreen']
        ...
        if args.is_sparse:
            plt.savefig(os.path.join(save_dir, 'compare_return_split_sparse.pdf'), dpi=300)
        else:
            plt.savefig(os.path.join(save_dir, 'compare_return_split.pdf'), dpi=300)
        # plt.show()
        ...
        plt.grid(axis='y', linestyle='--', alpha=0.5)
        if args.is_sparse:
            plt.savefig(os.path.join(save_dir, 'compare_return_full_sparse.pdf'), dpi=300)
        else:
            plt.savefig(os.path.join(save_dir, 'compare_return_full.pdf'), dpi=300)
