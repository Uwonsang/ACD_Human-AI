...
def overall_figure(result_dir, save_dir, args):
    ...
    if args.is_sparse:
        file_names = {
            'Ours': 'scientific_reports_3.pbt_return vs Proxy_sparse.csv',
             r'Ours$^{R}$': 'scientific_reports_3.pbt_return_coplayer_random vs Proxy_sparse.csv',
        }
    else:
        file_names = {
            'Ours': 'scientific_reports_3.pbt_return vs Proxy.csv',
             r'Ours$^{R}$': 'scientific_reports_3.pbt_return_coplayer_random vs Proxy.csv',
        }
    ...
    if args.figure_type == "split":
        ...
        num_groups = len(data)
        group_width = num_groups * bar_width + group_margin

        index = np.arange(len(layout_name)) * group_width
        plt.figure(figsize=(12, 6), dpi=300)

        colors = ['skyblue', 'plum', 'lightgreen']
        ...
        center_offset = (len(data) - 1) / 2 * bar_width
        plt.xticks(index + center_offset, layout_name, fontstyle='italic')
        plt.legend()
        plt.tight_layout()
        plt.grid(axis='y', linestyle='--', alpha=0.5)
        if args.is_sparse:
            plt.savefig(os.path.join(save_dir, 'sampling_split_sparse.pdf'), dpi=300)
        else:
            plt.savefig(os.path.join(save_dir, 'sampling_split.pdf'), dpi=300)
        # plt.show()
        ...
        plt.grid(axis='y', linestyle='--', alpha=0.5)
        if args.is_sparse:
            plt.savefig(os.path.join(save_dir, 'sampling_full_sparse.pdf'), dpi=300)
        else:
            plt.savefig(os.path.join(save_dir, 'sampling_full.pdf'), dpi=300)
        # plt.show()
...
if __name__ == "__main__":
    ...
    save_dir = '/app/analysis/figure/sampling/'
